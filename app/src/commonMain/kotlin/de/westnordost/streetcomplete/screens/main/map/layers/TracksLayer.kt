...
fun TracksLayers(
    ...
    val gpxData by produceState<Geometry>(EMPTY_GEOMETRY, gpxTrack) {
        value = withContext(Dispatchers.Default) { gpxTrack.toLineGeometry() ?: EMPTY_GEOMETRY }
    }
    val gpxSource = rememberGeoJsonSource(data = GeoJsonData.Features(gpxData))
    LineLayer(
        id = "gpx-layer",
        source = gpxSource,
        color = const(Color(0xff53fe70)),
        opacity = const(0.25f),
        width = const(5.dp),
