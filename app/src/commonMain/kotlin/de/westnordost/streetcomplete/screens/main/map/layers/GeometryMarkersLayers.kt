...
fun GeometryMarkersLayers(markers: Collection<Marker>, haloColor: Color, mapImages: MapImages) {
    ...
    FillLayer(
        ...
        opacity = const(0.3f),
        color = feature["color"].convertToColor(const(Color.GeometryMarker)),
    )
    LineLayer(
        id = "geo-lines",
        source = source,
    ...
    SymbolLayer(
        ...
        textColor = feature["color"].convertToColor(const(Color.GeometryMarker)),
    ...
    SymbolLayer(
        id = "geo-direction",
        source = source,
        filter = all(feature.isPoint(), feature.has("direction", true)),
        iconImage = image(painterResource(Res.drawable.location_view_direction)),
        iconColor = feature["color"].convertToColor(const(Color.GeometryMarker)),
        iconSize = byZoom(17 to 0.7f, 19 to 1.5f),
        iconAllowOverlap = const(true),
        iconRotate = feature["rotation"].convertToNumber(const(0)),
