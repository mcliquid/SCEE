...
abstract class MainMapViewModel : ViewModel() {
    ...
    /** Session-only reversal of the quest pin draw order. */
    abstract fun setReversedQuestOrder(reversed: Boolean)
...
class MainMapViewModelImpl(
    ...
    override fun setReversedQuestOrder(reversed: Boolean) {
        mapQuestPinsSource.setReversedOrder(reversed)
    }
    ...
    override val downloadedTiles = callbackFlow {
        ...
        withContext(Dispatchers.IO) {
            val retainMillis = prefs.getInt(
                Prefs.DATA_RETAIN_TIME,
                ApplicationConstants.DELETE_OLD_DATA_AFTER_DAYS,
            ) * 24L * 60 * 60 * 1000
            downloadedTilesSource.getAll(retainMillis)
        }
