                ...
                when (event) {
                    Event.Reload -> {
                        ...
                        if (selected != null) {
                            ...
                            val styledElements = mapData.toStyledElements(selected, levelFilter)
                    ...
                    is Event.Updated -> {
                        ...
                        if (selected != null) {
                            ...
                            val styledElements = event.updated.toStyledElements(selected, levelFilter)
                            for (styledElement in styledElements) {
                                if (bbox.intersect(styledElement.geometry.bounds) &&
                                    levelFilter.levelAllowed(styledElement.element)
                                ) {
...
private fun MapDataWithGeometry.toStyledElements(
    overlay: Overlay,
    levelFilter: LevelFilter,
): Sequence<StyledElement> =
    overlay.getStyledElements(this).mapNotNull { (element, style) ->
        if (!levelFilter.levelAllowed(element)) return@mapNotNull null
