...
private fun Edit.toProperties(): List<Pair<String, JsonPrimitive>> = when (this) {
    ...
    is ExternalSourceQuestHidden -> listOf(
        MARKER_EDIT_TYPE to JsonPrimitive(EDIT_TYPE_HIDE_OTHER_SOURCE_QUEST),
        MARKER_OTHER_SOURCE to JsonPrimitive(questType.source),
        MARKER_OTHER_SOURCE_ID to JsonPrimitive(id),
        MARKER_QUEST_TYPE to JsonPrimitive(questType.name),
    )
...
private fun JsonObject.toEditKey(): EditKey? {
    ...
    return when (editType) {
        ...
        EDIT_TYPE_HIDE_OTHER_SOURCE_QUEST ->
            QuestHiddenKey(ExternalSourceQuestKey(
                getValue(MARKER_OTHER_SOURCE_ID).jsonPrimitive.content,
