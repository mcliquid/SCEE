            ...
            text = {
                Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                    Button(
                        ...
                        Text(stringResource(Res.string.quest_settings_building_levels_mandatory_roof))
                    }
                    Button(
                        { prefs.putBoolean(questPrefix(prefs) + MANDATORY_ROOF_LEVELS, false); onDismissRequest() },
                        Modifier.fillMaxWidth()
                    ) {
                        Text(stringResource(Res.string.quest_settings_building_levels_optional_roof))
                    }
